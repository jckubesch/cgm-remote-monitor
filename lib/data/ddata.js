...
function init () {
  ...
  ddata.processTreatments = function processTreatments (preserveOrignalTreatments) {
    ...
    // filter & prepare 'Medtronic Reservoir Change' events
    // REMEMBER: Change this: ddata.insulinchangeTreatments
    ddata.reservoirTreatments = ddata.treatments.filter(function filterInsulin (t) {
      return t.eventType.indexOf('Medtronic Reservoir Change') > -1;
    }).sort(function(a, b) {
      return a.mills > b.mills;
    });
    
    // filter & prepare 'Long Acting Dose' events
    ddata.longactingTreatments = ddata.treatments.filter(function filterSensor (t) {
