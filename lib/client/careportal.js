...
function init (client, $) {
  ...
  careportal.prepare = function prepare () {
    ...
    $('#enteredBy').val(''); //Amber trying to remove 'undefined' autofill
  ...
  function gatherData () {
    ...
    var data = {
      enteredBy: $('#enteredBy').val() //commented out earlier 
      , eventType: eventType //commented out earlier 
      , otp: $('#otp').val() //commented out earlier after the comma 
  ...
  function buildConfirmText (data) {
    ...
    pushIf(data.enteredBy, translate('Entered By') + ': ' + data.enteredBy); //commented out earlier
